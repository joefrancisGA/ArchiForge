...
{
    /// <summary>
    ///     Creates a <see cref="RunRecord" />, ingests context, builds snapshots, runs
    ///     <see cref="Decisioning.Interfaces.IDecisionEngine.DecideAsync" />, synthesizes artifacts, commits the unit of work,
    ///     audits milestones, then best-effort semantic indexing.
    ...
    /// </summary>
    /// <param name="request">
    ///     Ingestion payload; <see cref="ContextIngestion.Models.ContextIngestionRequest.RunId" /> is set to
    ///     the new run id.
    /// </param>
    /// <param name="cancellationToken">Cancellation token.</param>
    /// <param name="evidenceBundleIdForDeferredWork">
    ///     When deferring, serialized into the work outbox so starter tasks
    ///     reference the same evidence bundle id.
    /// </param>
    /// <returns>
    ///     The persisted run with snapshot and manifest ids populated (or only <see cref="RunRecord.RunId" /> when
    ///     deferred).
    /// </returns>
    ...
    /// </summary>
    Task<RunRecord> CompleteQueuedAuthorityPipelineAsync(ContextIngestionRequest request,
        CancellationToken cancellationToken = default);
}
