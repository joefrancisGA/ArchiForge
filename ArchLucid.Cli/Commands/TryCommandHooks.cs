...
internal sealed class TryCommandHooks
{
    ...
    public required Func<string?> FindComposeDirectory { get; init; }

    /// <summary>Brings up the pilot Docker stack and waits for <c>/health/ready</c>; returns a CLI exit code.</summary>
    public required Func<CancellationToken, Task<int>> PilotUp { get; init; }

    /// <summary>POST /v1/demo/seed (best-effort).</summary>
    public required Func<string, CancellationToken, Task<TryCommand.DemoSeedOutcome>> DemoSeed { get; init; }

    /// <summary>Submit the sample architecture request via the API client.</summary>
    public required Func<ArchLucidApiClient, CancellationToken, Task<ArchLucidApiClient.CreateRunResult>> CreateRun { get; init; }

    /// <summary>POST /v1/architecture/run/{runId}/execute (best-effort).</summary>
    public required Func<string, string, CancellationToken, Task<bool>> ExecuteRun { get; init; }

    /// <summary>GET /v1/architecture/run/{runId} (used by the polling loop).</summary>
    public required Func<ArchLucidApiClient, string, CancellationToken, Task<ArchLucidApiClient.GetRunResult?>> GetRun { get; init; }

    /// <summary>POST /v1/architecture/run/{runId}/seed (Development-only fallback).</summary>
    public required Func<ArchLucidApiClient, string, CancellationToken, Task<ArchLucidApiClient.SeedFakeResultsResult?>> SeedFakeResults { get; init; }

    /// <summary>POST /v1/architecture/run/{runId}/commit.</summary>
    public required Func<ArchLucidApiClient, string, CancellationToken, Task<ArchLucidApiClient.CommitRunResult?>> CommitRun { get; init; }

    /// <summary>Save the first-value Markdown report to <c>savePath</c>; returns true on success.</summary>
    public required Func<string, string, string, CancellationToken, Task<bool>> DownloadFirstValueReport { get; init; }

    /// <summary>Open a local artifact (e.g. the saved Markdown) in the OS default handler.</summary>
    public required Action<string> OpenFile { get; init; }
    ...
    public required Func<string, ArchLucidApiClient> CreateApiClient { get; init; }
}
