  ...
  public static async Task<int> RunAsync(string[] args, CancellationToken cancellationToken = default)
  {
    ...
    if (CliExecutionContext.JsonOutput)
    {
      var payload = new
      {
        ...
        summary = new
        {
          requiredSatisfied, requiredTotal, optionalSet, optionalTotal
        },
        keys = lines
          .Select(
            c => new
            {
              configPath = c.ConfigPath, c.IsSet, c.Source, c.IsRequired, c.Notes
            })
          .ToList()
      };
    ...
    CancellationToken cancellationToken)
  {
    ...
    using HttpClient c = new()
    {
      BaseAddress = new Uri(
        baseUrl
          .Trim()
          .TrimEnd('/') + "/", UriKind.Absolute), Timeout = TimeSpan.FromSeconds(20)
    };
