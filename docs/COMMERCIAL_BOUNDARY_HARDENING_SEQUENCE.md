...
**Regression anchors:** **docs/PRODUCT_PACKAGING.md** §3 *Contributor drift guard* lists **Vitest** files that guard rank thresholds, nav composition (including default Enterprise **Reader** strip and **tier-before-authority** so Execute rank does not bypass extended disclosure), JWT `/me` refetch conservative rank (`OperatorNavAuthorityProvider.test.tsx`), cross-module Execute-link visibility (`authority-seam-regression.test.ts`), **`current-principal`** vs mutation-capability alignment, **LayerHeader** Enterprise rank cue coverage, and rank-gated Enterprise copy—extend that list when you add a new shaping surface tied to `/me`.
