...
PRs must pass the **blocking** merge gates (Tier **0–3b**, **2b**, **3c**, **3c′**, and **3d** as wired in `ci.yml`). Tier 2 (and 2b) are skipped automatically if Tier 1 fails (`needs: dotnet-fast-core`), saving SQL spin-up, full-suite time, and chaos runs on obvious breaks.
...
| **4e** | **[`live-e2e-nightly.yml`](../.github/workflows/live-e2e-nightly.yml)** (**Live E2E nightly**) | **03:30 UTC** (and `workflow_dispatch`): two jobs run the **full** `live-api-*.spec.ts` suite — once with DevelopmentBypass, once with ApiKey (forks skipped). **Not** a per-PR merge gate. See [LIVE_E2E_AUTH_PARITY.md](LIVE_E2E_AUTH_PARITY.md). |
