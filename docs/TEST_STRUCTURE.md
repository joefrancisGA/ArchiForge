...
**Live API E2E (Tier 2+ — CI `ui-e2e-live`, merge-blocking, `timeout-minutes: 35`):** `archlucid-ui/playwright.live.config.ts` uses **`testMatch: ["live-api-*.spec.ts"]`** (all files under `e2e/` matching that name). Includes: **`live-api-journey`** (happy path: manifest UI, export ZIP, governance approve, audit UI), **`live-api-conflict-journey`**, **`live-api-governance-rejection`**, **`live-api-negative-paths`**, **`live-api-error-states`**, **`live-api-advisory-flow`**, **`live-api-replay-export`**, **`live-api-analysis-report`**, **`live-api-policy-pack-lifecycle`**, **`live-api-compare-runs`**, **`live-api-alert-rules`** (create/list + `/alerts` axe), **`live-api-search-ask-graph`** (runs list, graph API, optional Ask, `/search` + `/ask` axe), **`live-api-digest-webhook`** (digest create/list/toggle + audit; skipped placeholder for webhook dry-run), **`live-api-concurrency`** (parallel commit + parallel governance approve), **`live-api-archival`** (skipped note for worker-only archival + multi-run list smoke). Default **`playwright.config.ts`** uses **`testIgnore: "**/live-api-*.spec.ts"`** so mock smoke never calls the real API. Helpers: **`e2e/helpers/live-api-client.ts`**. Narrative: **[LIVE_E2E_HAPPY_PATH.md](LIVE_E2E_HAPPY_PATH.md)**. Run locally: `npx playwright test -c playwright.live.config.ts` (API on **`LIVE_API_URL`**). CI sets **`LIVE_E2E_SKIP_NEXT_BUILD=1`**. See **[TEST_EXECUTION_MODEL.md](TEST_EXECUTION_MODEL.md)**.
