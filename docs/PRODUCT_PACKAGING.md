...
4. **Inline rank / reader hints** — **`EnterpriseControlsContextHints.tsx`** (e.g. **`GovernanceResolutionRankCue`**, **`AlertsInboxRankCue`**). **Same Execute numeric boundary** as (2) for copy, but a route may combine **(3)+(4)** with **(2)** on different paragraphs — e.g. **Governance resolution**: rank cue from **`useNavCallerAuthorityRank`**, **Change related controls** supplement from **`useEnterpriseMutationCapability()`**; **`enterprise-authority-ui-shaping.test.tsx`** locks the mutation branch.
