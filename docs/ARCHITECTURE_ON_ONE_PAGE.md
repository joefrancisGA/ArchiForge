...
**Purpose:** One screen to redraw **ArchLucid** as C4 and to know **who owns each box**, **where to read more**, and **which tests catch drift**. Deeper narrative: [ARCHITECTURE_CONTEXT.md](library/ARCHITECTURE_CONTEXT.md), [ARCHITECTURE_CONTAINERS.md](library/ARCHITECTURE_CONTAINERS.md), [ARCHITECTURE_COMPONENTS.md](library/ARCHITECTURE_COMPONENTS.md). Older flowchart-only view: [ARCHITECTURE_ON_A_PAGE.md](library/ARCHITECTURE_ON_A_PAGE.md).
...
| ArchLucid platform | `ArchLucid.Api`, `ArchLucid.Application`, `ArchLucid.Worker`, `ArchLucid.Host.Composition` | [ARCHITECTURE_CONTAINERS.md](library/ARCHITECTURE_CONTAINERS.md) | `ArchLucid.Api.Tests`, `ArchLucid.Application.Tests`, release smoke (`docs/RELEASE_SMOKE.md`) |
| SQL Server | `ArchLucid.Persistence` + `ArchLucid.Persistence.Data.*` + migrations | [DATA_MODEL.md](library/DATA_MODEL.md), [SQL_SCRIPTS.md](library/SQL_SCRIPTS.md) | SQL integration suites under `ArchLucid.*.Tests` |
| Azure OpenAI | `ArchLucid.Api` agent execution mode | [BUILD.md](library/BUILD.md) (config), agent docs in `docs/` | Simulator-first unit tests; live optional |
...
| ArchLucid.Api | `ArchLucid.Api/` | [ARCHITECTURE_COMPONENTS.md](library/ARCHITECTURE_COMPONENTS.md) § API | `ArchLucid.Api.Tests` |
| ArchLucid.Worker | `ArchLucid.Worker/` | [SYSTEM_MAP.md](library/SYSTEM_MAP.md) | Hosted-loop coverage via API / Decisioning tests + release smoke |
| ArchLucid.Cli | `ArchLucid.Cli/` | [CLI_USAGE.md](library/CLI_USAGE.md) | CLI integration / smoke scripts |
| SQL Server | `ArchLucid.Persistence*`, `Scripts/ArchLucid.sql` | [SQL_SCRIPTS.md](library/SQL_SCRIPTS.md) | Persistence + API integration |
| Azure OpenAI (ext) | Configuration + agent hosts | [CANONICAL_PIPELINE.md](library/CANONICAL_PIPELINE.md) | Decisioning + API tests (simulator default) |
...
1. **Create run** — `POST /v1/architecture/request` (wizard or CLI `archlucid run`). See [ARCHITECTURE_FLOWS.md § Flow A](library/ARCHITECTURE_FLOWS.md#flow-a-run-lifecycle-request--tasks--results-commit--manifest) step *Create run* / *Generate tasks*.
...
4. **Sponsor-facing PDF** — after artifacts exist: run detail exports or CLI `archlucid sponsor-one-pager <runId> [--save]`; Markdown / DOCX analysis exports follow [ARCHITECTURE_FLOWS.md § Flow B](library/ARCHITECTURE_FLOWS.md#flow-b-export-lifecycle-build--persist-record--replay).

**Where to go next:** [ARCHITECTURE_FLOWS.md](library/ARCHITECTURE_FLOWS.md) (full A/B/C narratives), [CORE_PILOT.md](CORE_PILOT.md) (first-pilot checklist), [OPERATOR_ATLAS.md](library/OPERATOR_ATLAS.md) (UI route × API × CLI map).
