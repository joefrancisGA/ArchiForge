...
The problem is that the route and vocabulary system still leaks implementation history. `docs/library/ONBOARDING_WIZARD.md` says `/getting-started` is canonical and `/onboarding` redirects to it, but the current UI implementation makes `/onboarding` canonical and redirects `/getting-started` to `/onboarding`. That is not a minor docs mismatch: it creates immediate doubt about which surface is current. The buyer-facing docs also say the product is a hosted SaaS, while several operator docs still assume local/API/CLI language close to the first-use path.
