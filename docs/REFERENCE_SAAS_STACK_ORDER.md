...
| 6 | `infra/terraform-openai` | Optional **budget** hooks for Azure OpenAI (resource creation may be out-of-band). |
| 7 | `infra/terraform-entra` | App registrations / consent text for API + UI. |
| 8 | `infra/terraform-container-apps` | **API + Worker + UI** workloads, managed identity wiring. |
| 9 | `infra/terraform-edge` | Front Door / WAF / routing to Container Apps. |
| 10 | `infra/terraform` | Optional **Consumption APIM** in front of public HTTPS backend — not a substitute for Premium VNet-injected APIM in all topologies. |
| 11 | `infra/terraform-monitoring` | Log Analytics, Grafana/Prometheus, alert rules, dashboards. |
| 12 | `infra/terraform-orchestrator` | Optional orchestration / automation root (if used in your fork). |
