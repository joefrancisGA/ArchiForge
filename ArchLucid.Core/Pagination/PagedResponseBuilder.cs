...
{
    /// <summary>
    ///     Applies in-memory skip/take over <paramref name="allItems" /> and wraps the result in a
    ...
    public static PagedResponse<T> Build<T>(IReadOnlyList<T> allItems, int page, int pageSize)
    {
        ...
        {
            Items = items, TotalCount = allItems.Count, Page = safePage, PageSize = safePageSize
        };
    }
        ...
        int pageSize)
    {
        ...
        return new PagedResponse<T>
        {
            Items = pageItems, TotalCount = totalCount, Page = safePage, PageSize = safePageSize
        };
    }
}
