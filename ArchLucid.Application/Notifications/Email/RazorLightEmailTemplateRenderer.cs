...
public sealed class RazorLightEmailTemplateRenderer : IEmailTemplateRenderer
{
    ...
    private readonly RazorLightEngine _engine = new RazorLightEngineBuilder().UseEmbeddedResourcesProject(typeof(EmailTemplateAnchor)).UseMemoryCachingProvider().Build();
    /// <inheritdoc/>
    ...
    {
        return string.IsNullOrWhiteSpace(templateId) ? throw new ArgumentException("Template id is required.", nameof(templateId)) : // RazorLight resolves views relative to <see cref="EmailTemplateAnchor"/>'s namespace (ArchLucid.Application.Notifications.Email).
