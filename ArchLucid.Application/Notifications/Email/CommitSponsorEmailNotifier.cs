    ...
    ILogger<CommitSponsorEmailNotifier> logger) : ICommitSponsorEmailNotifier
{
    ...
    public async Task NotifyAfterCommitAsync(Guid tenantId, string runId, CancellationToken cancellationToken)
    {
        ...
        if (string.IsNullOrWhiteSpace(to))
        {
            if (_logger.IsEnabled(LogLevel.Information))
                _logger.LogInformation(
                    "Commit sponsor email skipped: no admin mailbox for tenant {TenantId}, run {RunId}.",
                    tenantId,
                    trimmedRunId);
        ...
        string runUrlHref = operatorBase is null
            ? "#"
            : $"{operatorBase}/runs/{Uri.EscapeDataString(trimmedRunId)}";
        ...
        string html =
            $"<p>{safeProduct}: an architecture run was finalized.</p>" +
            $"<p>Run id: <strong>{safeRun}</strong></p>" +
        ...
        string text =
            $"{productName}: an architecture run was finalized.\n" +
            $"Run id: {trimmedRunId}\n" +
            $"Link: {runUrlText}\n";
        ...
        catch (Exception ex) when (!cancellationToken.IsCancellationRequested)
        {
            if (_logger.IsEnabled(LogLevel.Warning))
                _logger.LogWarning(
                    ex,
                    "Commit sponsor email send failed for tenant {TenantId}, run {RunId}.",
                    tenantId,
                    trimmedRunId);
        }
    }
