...
public sealed class TenantProvisioningService(ITenantRepository tenantRepository, IActorContext actorContext, IAuditService auditService, ILogger<TenantProvisioningService> logger) : ITenantProvisioningService
{
    private readonly byte __primaryConstructorArgumentValidation = __ValidatePrimaryConstructorArguments(tenantRepository, actorContext, auditService, logger);
    private static byte __ValidatePrimaryConstructorArguments(ArchLucid.Core.Tenancy.ITenantRepository tenantRepository, ArchLucid.Application.Common.IActorContext actorContext, ArchLucid.Core.Audit.IAuditService auditService, Microsoft.Extensions.Logging.ILogger<ArchLucid.Application.Tenancy.TenantProvisioningService> logger)
    {
    ...
    public async Task<TenantProvisioningResult> ProvisionAsync(TenantProvisioningRequest request, CancellationToken ct)
    {
        ...
        catch (Exception ex)
        {
            if (_logger.IsEnabled(LogLevel.Critical))
                _logger.LogCritical(ex, "Tenant {TenantId} inserted but default workspace insert failed; manual cleanup may be required.", tenantId);
            throw;
        }
