    ...
    : IConsultingDocxTemplateRecommendationService
{
        ...
        ConsultingDocxProfileRecommendationRequest request)
    {
        ...
        if (request.RegulatedEnvironment)
        {
            ...
            reason =
                "A regulated review was indicated, so a control-heavy report with stronger governance and appendix coverage is the best fit.";
        }
        else if (request.ExecutiveFriendly)
        {
            profile = Prefer(ConsultingDocxProfiles.Executive, available);
            reason =
                "An executive-friendly output was requested, so a shorter stakeholder-oriented brief is the best fit.";
        }
        ...
        else if (request.NeedDetailedEvidence || request.NeedExecutionTraces ||
                 request.NeedDeterminismOrCompareAppendices)
        {
            profile = Prefer(ConsultingDocxProfiles.Internal, available);
            reason =
                "Detailed evidence, traceability, or replay/comparison depth was requested, so the internal technical review profile is the best fit.";
        }
                  ...
                  request.Audience.Contains("regulator", StringComparison.OrdinalIgnoreCase)))
        {
            profile = Prefer(ConsultingDocxProfiles.Regulated, available);
            reason =
                "The audience indicates compliance, audit, or regulated review, so the regulated profile is the best fit.";
        }
        else if (!string.IsNullOrWhiteSpace(request.Audience) &&
                  ...
                  request.Audience.Contains("external", StringComparison.OrdinalIgnoreCase)))
        {
            profile = Prefer(ConsultingDocxProfiles.Client, available);
            reason =
                "The audience indicates an external or client-facing reader, so the client delivery profile is the best fit.";
        }
        else
        {
            profile = Prefer(ConsultingDocxProfiles.Client, available);
            reason =
                "No strong specialized signal was provided, so the balanced client delivery profile is the safest default.";
        }
