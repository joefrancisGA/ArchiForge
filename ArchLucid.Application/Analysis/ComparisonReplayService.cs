    ...
    : IComparisonReplayService
{
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        {
            ComparisonTypes.EndToEndReplay => await ReplayEndToEndAsync(record, format, profile, mode,
                cancellationToken),
        ...
        if (request.PersistReplay)
            ...
            result.PersistedReplayRecordId = await comparisonAuditService.RecordReplayOfAsync(
                record,
                $"Replay of comparison record {record.ComparisonRecordId} at {DateTime.UtcNow:O}.",
                cancellationToken);
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        ComparisonRecord record =
            await comparisonRecordRepository.GetByIdAsync(comparisonRecordId, cancellationToken) ??
        ...
        CancellationToken cancellationToken)
    {
        ...
        ReplayComparisonResult result =
            await BuildEndToEndResultAsync(record, report, format, profile, cancellationToken);
        ...
        CancellationToken cancellationToken)
    {
        if (string.IsNullOrWhiteSpace(record.LeftExportRecordId) ||
            string.IsNullOrWhiteSpace(record.RightExportRecordId))
        ...
        RunExportRecord left =
            await runExportRecordRepository.GetByIdAsync(record.LeftExportRecordId, cancellationToken)
            ?? throw new InvalidOperationException(
                $"Export record '{record.LeftExportRecordId}' was not found.");
        RunExportRecord right =
            await runExportRecordRepository.GetByIdAsync(record.RightExportRecordId, cancellationToken)
            ?? throw new InvalidOperationException(
