...
public static class PreCommitGateEvaluator
{
    ...
    /// </summary>
    public static PreCommitGateResult EvaluateForAssignment(IReadOnlyList<Finding> findings, PolicyPackAssignment enforcing, PreCommitGovernanceGateOptions options)
    {
        ArgumentNullException.ThrowIfNull(findings);
        ArgumentNullException.ThrowIfNull(enforcing);
        ArgumentNullException.ThrowIfNull(options);
        return Evaluate(findings, enforcing.BlockCommitOnCritical, enforcing.BlockCommitMinimumSeverity, enforcing.PolicyPackId.ToString("N"), options.WarnOnlySeverities);
    }

    /// <summary>
    ///     Evaluates proposed enforcement flags against findings (dry-run / what-if).
    /// </summary>
    public static PreCommitGateResult Evaluate(IReadOnlyList<Finding> findings, bool blockCommitOnCritical, int? blockCommitMinimumSeverity, string policyPackIdLabel, string[]? warnOnlySeverities)
    {
        ArgumentNullException.ThrowIfNull(findings);
