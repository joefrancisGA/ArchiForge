...
{
    public GovernanceApprovalReviewConflictException(string approvalRequestId, string attemptedOutcome, string currentStatus) : base(BuildMessage(approvalRequestId, attemptedOutcome, currentStatus))
    {
    ...
    private static string BuildMessage(string approvalRequestId, string attemptedOutcome, string currentStatus)
    {
        ...
        return $"Governance approval request '{approvalRequestId}' is no longer reviewable (current status '{currentStatus}'). " + $"Another request may have completed a concurrent {attemptedOutcome}.";
    }
}