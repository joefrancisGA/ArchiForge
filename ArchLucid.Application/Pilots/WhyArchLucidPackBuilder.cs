...
public static class WhyArchLucidPackBuilder
{
    ...
    private static string BuildIncumbentMarkdownTable()
    {
        ...
        (string Dimension, string Leanix, string Ardoq, string Mega, string Archlucid, string ArchCitation)[] rows =
        [
            (
                "AI capability",
        ...
        StringBuilder t = new();
        t.AppendLine("| Dimension | LeanIX (§2.1 paraphrase) | Ardoq (§2.1 paraphrase) | MEGA HOPEX (§2.1 paraphrase) | ArchLucid (V1; cited) | ArchLucid citations |");
        t.AppendLine("|-----------|--------------------------|-------------------------|------------------------------|----------------------|----------------------|");

        foreach ((string dimension, string leanix, string ardoq, string mega, string archlucid, string archCitation) in rows)
        {
            t.AppendLine(
                $"| {EscapePipe(dimension)} | {EscapePipe(leanix)} | {EscapePipe(ardoq)} | {EscapePipe(mega)} | {EscapePipe(archlucid)} | {EscapePipe(archCitation)} |");
        }
