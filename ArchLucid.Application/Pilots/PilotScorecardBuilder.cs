    ...
    ILogger<PilotScorecardBuilder> logger)
{
    ...
    /// <summary>Aggregates recent runs in scope; filters to <paramref name="periodStart"/> inclusive and <paramref name="periodEnd"/> exclusive (UTC).</summary>
    public async Task<PilotScorecardSummary> BuildAsync(
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        IReadOnlyList<RunRecord> recent = await _runRepository.ListRecentInScopeAsync(scope, take: 10_000, cancellationToken);

        DateTime startUtc = periodStart.UtcDateTime;
        DateTime endUtc = periodEnd.UtcDateTime;

        List<RunRecord> inWindow = recent
            .Where(r => r.CreatedUtc >= startUtc && r.CreatedUtc < endUtc)
            .ToList();
        ...
        if (_logger.IsEnabled(LogLevel.Information))
            _logger.LogInformation(
                "Pilot scorecard: tenant {TenantId}, window {Start:o}–{End:o}, runs {RunCount}, committed {Committed}.",
                scope.TenantId,
                startUtc,
                endUtc,
                inWindow.Count,
