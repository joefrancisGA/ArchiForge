...
public static class ContractGoldenManifestPersistence
{
        ...
        GoldenManifest? authorityPersistBody)
    {
        ...
        if (authorityPersistBody.ManifestId != keying.ManifestId)
            throw new ArgumentException("authorityPersistBody.ManifestId must match keying.ManifestId.", nameof(authorityPersistBody));

        if (authorityPersistBody.RunId != keying.RunId)
            throw new ArgumentException("authorityPersistBody.RunId must match keying.RunId.", nameof(authorityPersistBody));

        if (authorityPersistBody.ContextSnapshotId != keying.ContextSnapshotId)
            throw new ArgumentException("authorityPersistBody.ContextSnapshotId must match keying.ContextSnapshotId.", nameof(authorityPersistBody));

        if (authorityPersistBody.GraphSnapshotId != keying.GraphSnapshotId)
            throw new ArgumentException("authorityPersistBody.GraphSnapshotId must match keying.GraphSnapshotId.", nameof(authorityPersistBody));

        if (authorityPersistBody.FindingsSnapshotId != keying.FindingsSnapshotId)
            throw new ArgumentException("authorityPersistBody.FindingsSnapshotId must match keying.FindingsSnapshotId.", nameof(authorityPersistBody));

        if (authorityPersistBody.DecisionTraceId != keying.DecisionTraceId)
            throw new ArgumentException("authorityPersistBody.DecisionTraceId must match keying.DecisionTraceId.", nameof(authorityPersistBody));
