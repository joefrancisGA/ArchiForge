...
///         (for debugging and <c>GET …/policy-packs/effective</c>), distinct from
///         <see cref="IEffectiveGovernanceLoader" /> which returns a single merged document.
///     </para>
///     <para>
///         Primary caller: <c>ArchLucid.Api.Controllers.PolicyPacksController.GetEffective</c> and
///         <see cref="PolicyPackResolver" /> itself via DI.
///     </para>
/// </remarks>
public interface IPolicyPackResolver
{
    ...
    /// <returns>
    ///     <see cref="EffectivePolicyPackSet" /> with zero or more <see cref="ResolvedPolicyPack" /> entries; skipped rows
    ///     when pack or version missing.
    /// </returns>
    /// <remarks>
    ///     Does <strong>not</strong> apply hierarchical merge rules—only repository filtering and enabled flag. Merge
    ///     semantics live in
