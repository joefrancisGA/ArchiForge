...
/// <summary>
///     Persistence port for <see cref="PolicyPackAssignment" /> rows: create, toggle/update, and list by hierarchical
///     scope.
/// </summary>
/// <remarks>
...
public interface IPolicyPackAssignmentRepository
{
    ...
    ///     Entity to persist (typically built by <see cref="IPolicyPackManagementService.AssignAsync" />
    ///     ).
    /// </param>
    ...
    /// <param name="ct">Cancellation token.</param>
    /// <returns>
    ///     Matching rows, typically ordered by <see cref="PolicyPackAssignment.AssignedUtc" /> descending; may include
    ///     disabled rows.
    /// </returns>
    /// <remarks>
    ///     Consumers filter <see cref="PolicyPackAssignment.IsEnabled" /> themselves. Critical call paths:
    ///     <see cref="PolicyPackResolver.ResolveAsync" /> and
    ///     <see cref="Resolution.EffectiveGovernanceResolver.ResolveAsync" />.
    /// </remarks>
    ...
    /// <summary>
    ///     Returns the assignment row for the tenant and id, including archived rows, or <see langword="null" /> when
    ///     none exists.
    /// </summary>
    Task<PolicyPackAssignment?> GetByTenantAndAssignmentIdAsync(Guid tenantId, Guid assignmentId, CancellationToken ct);

    /// <summary>
    ///     Sets <see cref="PolicyPackAssignment.ArchivedUtc" /> when the row belongs to <paramref name="tenantId" /> and
