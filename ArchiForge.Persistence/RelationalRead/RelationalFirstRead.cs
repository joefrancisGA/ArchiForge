...
internal static class RelationalFirstRead
{
    ...
    /// <paramref name="policy"/> to decide whether to fall back to JSON columns.
    /// </summary>
    /// <param name="relationalRowCount">COUNT(1) from the relational child table.</param>
    /// <param name="sliceName">Diagnostic label passed to <see cref="JsonFallbackPolicy.ShouldFallbackToJson"/>.</param>
        ...
        JsonFallbackPolicy? policy)
    {
        ...
        if (policy is null || policy.ShouldFallbackToJson(relationalRowCount, sliceName))
