# `ArchiForge.Persistence/Migrations`

SQL Server **incremental** migrations consumed by **DbUp** at API startup.

- **Embedded** in assembly **ArchiForge.Persistence** (`EmbeddedResource Include="Migrations\*.sql"` in `ArchiForge.Persistence.csproj`).
- **Ordering:** Resource names must sort lexicographically — use prefixes `001_`, `002_`, … (see `ArchiForge.Api.Tests/DatabaseMigrationScriptTests.cs`).
