        ...
        CancellationToken ct)
    {
        ...
        for (int w = 0; w < snapshot.Warnings.Count; w++)
            ...
            await connection.ExecuteAsync(
                new CommandDefinition(
                    insertWarningSql,
                    new { snapshot.SnapshotId, SortOrder = w, WarningText = snapshot.Warnings[w] },
                    transaction,
                    cancellationToken: ct));
    }
        ...
        CancellationToken ct)
    {
        ...
        for (int e = 0; e < snapshot.Errors.Count; e++)
            ...
            await connection.ExecuteAsync(
                new CommandDefinition(
                    insertErrorSql,
                    new { snapshot.SnapshotId, SortOrder = e, ErrorText = snapshot.Errors[e] },
                    transaction,
                    cancellationToken: ct));
    }
        ...
        CancellationToken ct)
    {
        ...
        for (int h = 0; h < orderedHashes.Count; h++)
        {
            ...
            await connection.ExecuteAsync(
                new CommandDefinition(
                    insertHashSql,
                    new { snapshot.SnapshotId, SortOrder = h, SourceKey = kv.Key, HashValue = kv.Value },
                    transaction,
                    cancellationToken: ct));
        }
    }
