/*
  R108 (rollback of 108): re-create the legacy `rls.Archiforge*` predicate functions and
  `rls.ArchiforgeTenantScope` security policy, restoring the historical `af_*`
  SESSION_CONTEXT key contract. Drops the new `rls.archlucid_*` / `rls.ArchLucidTenantScope`
  objects.

  Pair with rolling back the application-layer change in
  `ArchLucid.Persistence/Connections/RlsSessionContextApplicator.cs` so the runtime sets
  `af_*` keys again — otherwise the restored predicates evaluate to "deny" and every
  authority query returns zero rows.

  See 108_RlsRenameToArchLucid.sql for the inverse direction and the rationale for the
...
IF @PreviousState = 0 AND EXISTS (SELECT 1 FROM sys.security_policies WHERE name = N'ArchiforgeTenantScope')
    SELECT @PreviousState = is_enabled
    FROM sys.security_policies
...
EXEC sp_set_session_context @key = N'__archlucid_R108_previous_state', @value = @PreviousState, @read_only = 0;
GO
...
DECLARE @Restore BIT = TRY_CONVERT(BIT, SESSION_CONTEXT(N'__archlucid_R108_previous_state'));

IF @Restore = 1
    ALTER SECURITY POLICY rls.ArchiforgeTenantScope WITH (STATE = ON);

EXEC sp_set_session_context @key = N'__archlucid_R108_previous_state', @value = NULL, @read_only = 0;
GO
