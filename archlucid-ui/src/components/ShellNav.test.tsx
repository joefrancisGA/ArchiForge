...
describe("ShellNav (55R smoke — primary navigation)", () => {
  it("exposes start-and-review workflow links with expected routes", () => {
    ...
    const nav = screen.getByRole("navigation", { name: "Start and review workflows" });
    ...
    expect(screen.getByRole("link", { name: "Compare runs" })).toHaveAttribute("href", "/compare");
    expect(screen.getByRole("link", { name: "Replay run" })).toHaveAttribute("href", "/replay");
  });
